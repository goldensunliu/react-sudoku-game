...
// TODO use immutable when this is all working, I know something is gonna break, be ready to manage stress
export function makePuzzle() {
    while(true) {
        try {
            const puzzle = Array.from(Array(9).keys()).map(i => Array.from(Array(9).keys()))
            const rows = Array.from(Array(9).keys()).map(i => new Set([1, 2, 3, 4, 5, 6, 7, 8, 9]))
            const columns = Array.from(Array(9).keys()).map(i => new Set([1, 2, 3, 4, 5, 6, 7, 8, 9]))
            const squares = Array.from(Array(9).keys()).map(i => new Set([1, 2, 3, 4, 5, 6, 7, 8, 9]))
...
export function pluck(puzzle, n=0) {
    /**
...
function canBeA(puzzle, i, j, c) {
    ...
    if (0 < puzzle[i][j]) return false
    // if not the cell itself, and the mth cell of the group contains the value v, then "no"
    for (const m in Array.from(Array(9).keys())) {
        const rowPeer = { x: m, y: j };
        const columnPeer = { x: i, y: m };
        const SquarePeer = { x: Math.floor(i/3)*3 + Math.floor(m/3), y: Math.floor(j/3)*3 + m%3 }
